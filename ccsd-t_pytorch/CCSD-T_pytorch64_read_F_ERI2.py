...
def n2t(A):
    return torch.from_numpy(A).double().to(device)
...
for CCSD_iter in range(1, maxiter + 1):
    ...
    CCSDcorr_E += 0.25 * torch.einsum('ijab,ijab->', MO2ijab, t2)
    CCSDcorr_E += 0.5 * torch.einsum('ijab,ia,jb->', MO2ijab, t1, t1)
...
t = time.time()
tmp = torch.einsum('ia,jkbc->ijkabc', t1, MO2[o, o, v, v])
t3d = tmp.clone()
t3d -= torch.einsum('ijkabc->ijkbac', tmp)
t3d -= torch.einsum('ijkabc->ijkcba', tmp)
t3d -= torch.einsum('ijkabc->jikabc', tmp)
t3d += torch.einsum('ijkabc->jikbac', tmp)
t3d += torch.einsum('ijkabc->jikcba', tmp)
t3d -= torch.einsum('ijkabc->kjiabc', tmp)
t3d += torch.einsum('ijkabc->kjibac', tmp)
t3d += torch.einsum('ijkabc->kjicba', tmp)
print('...built disconnected T3 in %.3f seconds.' % (time.time() - t))

# Build connected t3
print('\nBuilding connected T3...')
t = time.time()
tmp = torch.einsum('jkae,eibc->ijkabc', t2, MO2[v, o, v, v]).clone()
tmp -= torch.einsum('imbc,majk->ijkabc', t2, MO2[o, v, o, o])
t3c = tmp.clone()
t3c -= torch.einsum('ijkabc->ijkbac', tmp)
t3c -= torch.einsum('ijkabc->ijkcba', tmp)
t3c -= torch.einsum('ijkabc->jikabc', tmp)
t3c += torch.einsum('ijkabc->jikbac', tmp)
t3c += torch.einsum('ijkabc->jikcba', tmp)
t3c -= torch.einsum('ijkabc->kjiabc', tmp)
t3c += torch.einsum('ijkabc->kjibac', tmp)
t3c += torch.einsum('ijkabc->kjicba', tmp)
...
# Construct D3
Focc = torch.diag(F[o, o])
Fvir = torch.diag(F[v, v])
...
Pert_T = (1.0/36) * torch.einsum('ijkabc,ijkabc', t3c, tmp)

print('\nFinal CCSD(T) correlation energy:     % 16.10f' % (CCSDcorr_E+Pert_T))
